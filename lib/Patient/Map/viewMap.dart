...
import 'package:fyp/Patient/Map/viewClinicSpecialsist.dart';
...
class MapLocation extends StatefulWidget {
  ...
  final int patientID;
  MapLocation({required this.patientID});
...
class _MapLocationState extends State<MapLocation> {
  ...
  Future<List<LatLng>> getDirections(
    ...
    final apiKey = 'AIzaSyC7kR5dr435HsBg3PKkrwDrD1h6V3zrrI4';
    final apiUrl =
  ...
  Future<void> fetchMarkers() async {
    try {
      final response = await http.get(
          Uri.parse('http://${MyApp.ipAddress}/mediplexity/clinic.php'));
              ...
              Container(
                ...
                child: ElevatedButton(
                  onPressed: () async {
                    ...
                    var clinicID = int.parse(item['clinicID']);
                    await pref.setInt("clinicID", clinicID);
                    print('Tapped on clinic: $clinicID');

                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => viewClinicSpecialist(clinicID: clinicID,patientID: patientID, ),
                      ),
                    );
                  },
  ...
  Future<Map<String, String>> getDistanceAndDuration(
    ...
    final apiKey = 'AIzaSyDdqnV_yiYt9qUYIuAyBQOwY-QDF59z8ww';
    final apiUrl =
        'https://maps.googleapis.com/maps/api/directions/json?'
        'origin=${origin.latitude},${origin.longitude}&'
        'destination=${destination.latitude},${destination.longitude}&'
  ...
  Future<void> updateSuggestions(String query) async {
    final response = await http.get(
        Uri.parse('http://${MyApp.ipAddress}/mediplexity/clinic.php/suggestions?q=$query'));
